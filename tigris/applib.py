...
class BaseHandler(webapp2.RequestHandler):
  ...
  PROJECTED_FIELD_A = ['fid','filename','author','public','protected', 'date']
  ...
  def _query_list(self, maxium=20, query_public=False):
      if query_public:
          query = CodeBase.query(CodeBase.public == True).order(-CodeBase.date)
          return query.fetch(maxium, projection=self.PROJECTED_FIELD_B)
      else:
          query = CodeBase.query(ancestor=ndb.Key('CodeBase', self.user.name)).order(-CodeBase.date)
